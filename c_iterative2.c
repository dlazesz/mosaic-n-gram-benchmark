// Compile: gcc -O3 -march=native -flto -pipe -o c_iterative2 c_iterative2.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <getopt.h>
...
// Iterative permutation generator: writes directly to fout
void permute_iterative_fwrite(WordSplits *words, int maxn, FILE *fout) {
    ...
    while (1) {
        // Print n-gram directly
        for (int i = 0; i < maxn; i++) {
            if (i > 0) fputc(' ', fout);
            fwrite(words[i].subwords[indices[i]], 1, words[i].len[indices[i]], fout);
        }
        fputc('\n', fout);
...
void generate_ngrams(char *line, int maxn, const char *delimiter, const char *padding_token, FILE *fout) {
    ...
    for (int i = 0; i <= padded_count - maxn; i++) {
        permute_iterative_fwrite(&word_splits[i], maxn, fout);
    }
}
